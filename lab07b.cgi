...
if ($first_name eq '' || $first_name !~ /^[a-zA-Z]+$/) {
    $error_message .= "(Invalid or Missing First Name.) ";
}


if ($last_name eq '' || $last_name !~ /^[a-zA-Z]+$/) {
    $error_message .= "(Invalid or Missing Last Name.) ";
}


if ($street_name eq '') {
    $error_message .= "(Street Name is mandatory.) ";
}
...
if ($postal_code eq '' || $postal_code !~ /^[A-Za-z]\d[A-Za-z] \d[A-Za-z]\d$/) {
    $error_message .= "(Invalid or Missing Postal Code.) ";
}


if ($province eq '') {
    $error_message .= "(Province is mandatory.) ";
}
...
if (!defined $photo_extension || !grep { lc($photo_extension) eq $_ } @allowed_extensions) {
    $error_message .= "(Invalid Photo Format. Upload a .png, .gif, .jpg, or .jpeg file.) ";
}
...
if ($error_message) {
    ...
    print "<h2><b>&#128128 Told you! Consequences (Try Again) &#128128</b></h2>";
    ...
    print "<div class='container'>";
    print "<div class='display-info'>";
    print "<h2>&#128175 Amazing! (Double Check Your Informations!) &#128175</h2>";
